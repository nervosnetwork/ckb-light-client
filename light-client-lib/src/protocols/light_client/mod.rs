...
use ckb_network::{
    async_trait, bytes::Bytes, BoxedCKBProtocolContext, CKBProtocolContext, CKBProtocolHandler,
...
use crate::storage::db::{StorageBatchRelatedOperations, StorageGeneralOperations, StorageGetPinnedRelatedOperations, StorageHighLevelOperations};
...
impl CKBProtocolHandler for LightClientProtocol {
    ...
    async fn notify(&mut self, nc: BoxedCKBProtocolContext, token: u64) {
        match token {
            constant::REFRESH_PEERS_TOKEN => {
                self.refresh_all_peers(&nc).await;
            }
            constant::FETCH_HEADER_TX_TOKEN => {
                self.fetch_headers_txs(&nc).await;
            }
            constant::GET_IDLE_BLOCKS_TOKEN => {
                self.get_idle_blocks(&nc).await;
            }
            _ => unreachable!(),
        }
    }
...
impl LightClientProtocol {
    async fn try_process(
        ...
        match message {
            packed::LightClientMessageUnionReader::SendLastState(reader) => {
                components::SendLastStateProcess::new(reader, self, peer_index, nc)
                    .execute()
                    .await
            }
            packed::LightClientMessageUnionReader::SendLastStateProof(reader) => {
                components::SendLastStateProofProcess::new(reader, self, peer_index, nc)
                    .execute()
                    .await
            }
            packed::LightClientMessageUnionReader::SendBlocksProof(reader) => {
                components::SendBlocksProofProcess::new(reader, self, peer_index, nc)
                    .execute()
                    .await
            }
        ...
        if let Some(last_state) = peer_state.get_last_state() {
                ...
                .await
            {
                ...
                let message = packed::LightClientMessage::new_builder()
                    .set(content.clone())
                    .build();
                nc.as_ref().reply(peer_index, &message);
    ...
    pub(crate) async fn commit_prove_state(
        &self,
        peer_index: PeerIndex,
        new_prove_state: ProveState,
    ) -> Result<bool, Status> {
...
impl LightClientProtocol {
    pub fn new(storage: Storage, peers: Arc<Peers>, consensus: Consensus) -> Self {
    ...
    async fn refresh_all_peers(&mut self, nc: &BoxedCKBProtocolContext) {
        ...
        for index in self.peers().get_peers_which_require_new_state(before_ts) {
            if let Err(err) = self.get_last_state(nc.as_ref(), index) {
                error!(
                    "failed to request last state from peer={} since {}",
                    index, err
                );
        ...
        for index in self.peers().get_peers_which_require_new_proof() {
            if let Err(err) = self.get_last_state_proof(nc, index).await {
                error!(
        ...
        self.finalize_check_points(nc).await;
    }

    async fn finalize_check_points(&mut self, nc: &BoxedCKBProtocolContext) {
    ...
    pub(crate) async fn build_prove_request_content(
        &self,
        ...
        let (start_hash, start_number, start_total_difficulty) =
            match peer_state.get_prove_state().map(|inner| {
                ...
                Some((start_hash, start_number, start_total_difficulty)) => {
    ...
    pub(crate) async fn build_prove_request_content_from_genesis(
        &self,
        last_header: &VerifiableHeader,
    ) -> Option<packed::GetLastStateProof> {
        let last_n_blocks = self.last_n_blocks();
