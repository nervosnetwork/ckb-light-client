...
mod native;
#[cfg(not(target_arch = "wasm32"))]
pub use native::{Batch, CursorDirection, Storage};
...
impl Storage {
    ...
    fn clear_matched_blocks(&self) {
        ...
        let value = self.collect_iterator_key(
            key_prefix.clone(),
            CursorDirection::NextUnique,
    ...
    pub fn get_max_check_point_index(&self) -> CpIndex {
        ...
        self.get_pinned(&key)
            ...
            .map(|data| CpIndex::from_be_bytes(data.try_into().unwrap()))
            .expect("db get max check point index should be ok")
    }
