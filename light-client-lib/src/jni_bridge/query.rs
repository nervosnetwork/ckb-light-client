...
pub extern "C" fn Java_com_nervosnetwork_ckblightclient_LightClientNative_nativeGetTipHeader(
    ...
    let binding = STORAGE_WITH_DATA.read().unwrap();
    let swc = match binding.as_ref() {
        Some(s) => s,
        None => {
...
pub extern "C" fn Java_com_nervosnetwork_ckblightclient_LightClientNative_nativeFetchHeader(
    ...
    let peers_binding = PEERS.read().unwrap();
    let peers = match peers_binding.as_ref() {
        Some(p) => p,
        None => {
    ...
    let fetch_status: FetchStatus<HeaderView> =
        if let Some(header) = swc.storage().get_header(&hash) {
            ...
            // Add to fetch queue
            let net_binding = NET_CONTROL.read().unwrap();
            let _net_controller = match net_binding.as_ref() {
                Some(nc) => nc,
                None => {
...
pub extern "C" fn Java_com_nervosnetwork_ckblightclient_LightClientNative_nativeLocalNodeInfo(
    ...
    let net_binding = NET_CONTROL.read().unwrap();
    let net_controller = match net_binding.as_ref() {
        Some(nc) => nc,
        None => {
            error!("Network controller not initialized");
            return ptr::null_mut();
    ...
    let consensus_binding = CONSENSUS.read().unwrap();
    let _consensus = match consensus_binding.as_ref() {
        Some(c) => c,
        None => {
