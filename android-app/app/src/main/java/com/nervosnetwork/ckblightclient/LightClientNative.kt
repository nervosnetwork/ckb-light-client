...
object LightClientNative {
     ...
     * @param logCallback Callback for log messages (batched every 100ms)
     ...
     *
     * Receives batched log messages every 100ms.
     * Called from native thread, so must be thread-safe.
     */
    interface LogCallback {
        /**
         * Called when log messages are available
         *
         * @param level Log level ("ERROR", "WARN", "INFO", "DEBUG", "TRACE")
         * @param messages Array of log messages (batched)
         */
        fun onLog(level: String, messages: Array<String>)
    }
