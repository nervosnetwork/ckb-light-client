...
pub struct BinarySearchApproach {
    honest_peer: Option<PeerIndex>,
    peers: Peers,
    proofs: HashMap<PeerIndex, (Vec<BlockNumber>, Option<bool>)>,
}

impl BlockSamplingStrategy for BinarySearchApproach {
    fn new() -> Self {
        Self {
            honest_peer: None,
            peers: Peers::default(),
    ...
    fn start(&mut self, nc: &dyn CKBProtocolContext) {
        ...
        for (peer, state) in peers {
            ...
            let numbers = {
                let mut curr_number = (last_number + mmr_activated_number - 1) / 2;
                let mut tmp = vec![curr_number];
                while curr_number < last_number {
                    let next_number = (curr_number + last_number - 1) / 2;
                    if curr_number == next_number {
                        break;
                    }
                    curr_number = next_number;
                    tmp.push(curr_number);
                }
                tmp.sort();
                tmp.dedup();
                tmp
