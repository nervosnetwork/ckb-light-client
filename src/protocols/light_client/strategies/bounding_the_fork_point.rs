...
pub struct BoundingTheForkPoint {
    honest_peer: Option<PeerIndex>,
    peers: Peers,
    proofs: HashMap<PeerIndex, (Vec<BlockNumber>, Option<bool>)>,
}

impl BlockSamplingStrategy for BoundingTheForkPoint {
    fn new() -> Self {
        Self {
            honest_peer: None,
            peers: Peers::default(),
    ...
    fn start(&mut self, nc: &dyn CKBProtocolContext) {
        ...
        for (peer, state) in peers {
            ...
            let numbers = {
                ...
                let mut start_number = mmr_activated_number;
                let mut remain_count = last_number - start_number;

                while (remain_count as usize) > K_SAMPLES {
                    let mut tmp_k = (start_number..last_number).collect::<Vec<_>>();
                    let mut rng = thread_rng();
                    tmp_k.shuffle(&mut rng);
                    tmp_k.truncate(K_SAMPLES);

                    tmp.extend_from_slice(&tmp_k);

                    start_number = (start_number + last_number) / 2;
                    remain_count = last_number - start_number;
                }

                for n in start_number..last_number {
                    tmp.push(n);
                }
